...
export async function GET(request: NextRequest) {
  try {
    ...
    const url = `${API_BASE}/profile/${platformEndpoint}/${playerId}/stats/overview/kdRatio`;
    ...
    if (!response.ok) {
      return NextResponse.json(
        { error: `Failed to fetch K/D ratio: ${response.statusText}` },
    ...
    console.error('Error fetching K/D ratio:', error);
    return NextResponse.json(
