...
export async function GET(request: NextRequest) {
  try {
    ...
    const url = `${API_BASE}/profile/${platformEndpoint}/${playerId}/stats/overview/wlPercentage`;
    ...
    if (!response.ok) {
      return NextResponse.json(
        { error: `Failed to fetch win/loss percentage: ${response.statusText}` },
    ...
    console.error('Error fetching win/loss percentage:', error);
    return NextResponse.json(
